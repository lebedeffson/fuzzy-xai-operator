...
def score_rows(rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    decision_and_action_all = all(
        _boolean(row["decision_correct"]) and _boolean(row["action_correct"])
        for row in by_mode["human_explanation"]
    )
    passed = (
        decision_and_action_all
        and human["all_five_correct_rate"] >= 0.8
