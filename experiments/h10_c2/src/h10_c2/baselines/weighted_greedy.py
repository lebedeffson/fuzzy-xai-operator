...
def run(case: Case) -> MethodResult:
    ...
    while remaining:
        ...
        selected = min(
            atoms,
            key=lambda atom: (
                case.repair_costs.get(atom, 1.0) / max(1, sum(atom in path for path in remaining)),
                atom,
            ),
        )
    ...
    return MethodResult(case.case_id, case.pipeline, "weighted_greedy", cut, sum(case.repair_costs.get(atom, 1.0) for atom in cut), optimality_claimed=False, runtime_ms=(perf_counter() - started) * 1000)
