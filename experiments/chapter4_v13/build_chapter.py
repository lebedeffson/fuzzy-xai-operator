...
![Рисунок 4.25 — Распределение показателей локального объяснения в современном текстовом контуре]({(figures / 'explanation_quality.png').resolve()}){{width=15cm}}
...
{_markdown_table(policies, {'policy': 'Политика', 'automatic_coverage': 'Автоматическое покрытие', 'wrong_automatic_actions': 'Ошибочные автоматические действия', 'selective_risk': 'Селективный риск', 'manual_review_load': 'Объекты на проверке', 'false_blocks': 'Ложные блокировки', 'total_cost': 'Стоимость', 'risk_auroc': 'AUROC риска', 'risk_auprc': 'AUPRC риска', 'expected_calibration_error': 'Ошибка калибровки', 'brier_score': 'Brier'})}
...
![Рисунок 4.26 — Риск и автоматическое покрытие политик при одинаковых бюджетах проверки]({(figures / 'policy_risk_coverage.png').resolve()}){{width=15cm}}
...
{_markdown_table(route, {'group': 'Группа', 'method': 'Метод', 'n': 'N', 'precision': 'Точность', 'recall': 'Полнота', 'f1': 'F1', 'false_certification': 'Ложная сертификация', 'false_rejection': 'Ложный отказ', 'fault_type_accuracy': 'Тип нарушения', 'component_localization_accuracy': 'Локализация', 'diagnostic_time_ms_mean': 'Время, мс'})}

Таблица 4.39 — Диагностика одиночных, композиционных и отложенных нарушений

![Рисунок 4.27 — Качество обнаружения нарушений для четырёх проверяющих схем]({(figures / 'route_faults.png').resolve()}){{width=15cm}}

Рисунок 4.27 — Качество обнаружения нарушений для четырёх проверяющих схем

### 4.26.5 Полная стоимость конвейера

Время разложено на модель, внешний объяснитель, собственный слой FuzzyXAI и сериализацию. Выполнено по {int(manifest['repetitions'])} измерений после прогрева. Окружение: {runtime_env['gpu']}; Python {runtime_env['python']}; PyTorch {runtime_env['packages']['torch']}; доступно {int(runtime_env['cpu_count'])} вычислительных потоков. Показатели прежнего опыта на 5 млн записей по-прежнему относятся только к кэшированному операторному слою и не используются как характеристика полного конвейера.

{_markdown_table(runtime, {'explainer': 'Объяснитель', 'n': 'N', 'repetitions': 'Повторы', 'model_seconds_median': 'Модель, с', 'explainer_seconds_median': 'Объяснитель, с', 'fuzzyxai_seconds_median': 'FuzzyXAI, с', 'serialization_seconds_median': 'Сериализация, с', 'total_seconds_median': 'Полное время, с', 'objects_per_second_median': 'Объектов/с', 'peak_rss_bytes_median': 'Пиковая RAM, байт', 'peak_vram_bytes_median': 'Пиковая VRAM, байт', 'fuzzyxai_time_fraction': 'Доля FuzzyXAI', 'explainer_time_fraction': 'Доля объяснителя'})}

Таблица 4.40 — Сквозная вычислительная стоимость современного текстового контура

![Рисунок 4.28 — Декомпозиция полного времени по этапам конвейера]({(figures / 'runtime_decomposition.png').resolve()}){{width=16cm}}
...
Таблица 4.41 — Состав воспроизводимого сквозного примера

### 4.26.7 Статус новых проверок

{_markdown_table(hypotheses, {'hypothesis': 'Проверка', 'result': 'Результат', 'scope': 'Область'})}

Таблица 4.42 — Статус проверок дополнительного контура v13
